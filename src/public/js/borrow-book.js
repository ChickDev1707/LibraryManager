...
function submit() {
    ...
    if (!checkDate(ngay_muon)) {
        const result = { success: false, message: "Ngày mượn không được vượt quá thời gian hiện tại!" }
        showToast(result)
        return;
    }
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.error) {
                const messageToast = { success: false, message: result.message };
...
function subConfirm() {
    ...
    if (!checkDate(ngayMuon)) {
        const result = { success: false, message: "Ngày lấy không được vượt quá thời gian hiện tại!" };
        showToast(result);
        return;
    }
    $.ajax({
