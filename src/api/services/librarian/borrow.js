...
async function checkBorrowHistory(readerId, bookId) {
    ...
    try {
        ...
        console.log(err);
        return { ok: false, message: "Lỗi kiểm tra thông tin độc tài khoản!" };;
    }
}
    ...
    for (let i = 0; i < dsDauSach.length; i++) {
        try {
            ...
            if (book.so_luong_kha_dung == 0) {
                nErrors.push({ success: 0, updated: 0, message: `Sách đã được mượn hết `, ten_dau_sach: book.ten_dau_sach, ma_sach: dsMaSach[i] });
                continue;
            }
...
async function getBookBorrowByID(readerId, childId) {
    ...
    if (book == null)
        ...
        if (book.so_luong_kha_dung == 0)
            return { success: false, book: null, message: "Sách đã được mượn hết!" }
