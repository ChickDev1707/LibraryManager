...
async function getUserProfile(req, res, next){
    var currentUser = await getCurrentUserAccount(req);
    if(!currentUser)
       return res.redirect('/')
    var userProfile  = await getProfileByAccountId(currentUser._id);
...
async function updateUser(req, res, next){
    ...
    if(req.body.anh_bia){
        const result = await updateAvatar(currentUser._id, req.body.anh_bia)
        ...
        const result = await updateUserProfile(currentUser._id, newData);
        return res.json(result);
    }else{
        ...
        var result = await updatePassword(currentUser._id, currentPass, newPass);
        return res.json(result);
    }
